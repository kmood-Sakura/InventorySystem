...
int update_item_quantity(const char* directory,const char* filename, const char* item_id, int quantity_change) {
    ...
    char *temp_filepath = create_current_path(directory,"temp","csv");
    ...
    while (fgets(line, MAX_GOODS_DATA_LEN, file) != NULL) {
        ...
        if (strcmp(id, item_id) == 0) {
            ...
            if (new_quantity < 0) {
                printf("Error: Not enough goods quantity.\n");
                fclose(file);
                fclose(temp);
...
int update_item_quantitypath(const char* directory,const char* filepath, const char* item_id, int quantity_change) {
